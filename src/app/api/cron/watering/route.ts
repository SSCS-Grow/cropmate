...
/**
 * Sikkerhed: Kun kald med Authorization: Bearer <CRON_SECRET> (eller ?token=)
 */
function checkAuth(req: NextRequest) {
  ...
  if (!process.env.CRON_SECRET) {
    return "Missing CRON_SECRET env";
  }
  if (auth === expected || qsToken === process.env.CRON_SECRET) {
    return null;
  }
  return "Unauthorized";
}
...
async function runWatering() {
  ...
  for (const p of profiles || []) {
    try {
      ...
      // videre til næste profil
    }
  }

  return { ok: true, users_checked: touchedUsers, tasks_created: createdTasks, history_upserts: historyUpserts };
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await runWatering();
...
/** GET (manuelt test i browser/cURL – kræver ?token= eller Authorization header) */
