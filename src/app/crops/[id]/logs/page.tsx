...
useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!note && !file) { alert('Skriv en note eller vælg et billede.'); return }
    setSubmitting(true)
    try {
      ...
      console.error(e)
      alert('Kunne ikke gemme logbogen. Tjek at du er logget ind og at Storage bucket "logs" findes.')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm opacity-70">
            <Link href={`/crops/${crop.id}`} className="underline decoration-slate-300 hover:decoration-slate-900">
      ...
      <section className="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 p-4">
        ...
        {!logs.length && <p className="text-sm opacity-70">Ingen logs endnu. Tilføj din første ovenfor 🌱</p>}
            ...
            <li key={log.id} className="p-3 rounded-lg border border-slate-200">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <div className="text-sm opacity-70">{log.entry_date || (log.created_at?.slice(0,10))}</div>
                  {log.text ? <p className="text-sm mt-1 whitespace-pre-line">{log.text}</p> : <p className="text-sm opacity-60">—</p>}
