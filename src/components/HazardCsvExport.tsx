...
export default function HazardCsvExport({ hazardId }: { hazardId: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!alive) return;
      if (error) {
        console.info('export headers', error);
        return;
      }
  ...
  async function handleExport(allFiltered: boolean) {
    ...
    try {
      ...
      let q = supabase
        .from('hazard_reports')
        .select(
          'id, created_at, severity, latitude, longitude, photo_url, crop_id, crops(name), user_id',
        )
        .eq('hazard_id', hazardId)
      ...
      a.href = url;
      a.download = `hazard_${hazardId}_reports_${tag}${
        allFiltered ? '' : '_mine'
      }.csv`;
      ...
      console.info('exported rows', rows.length);
  ...
  return (
    <div className="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 p-4 grid gap-3">
      ...
      <p className="text-xs opacity-70">
        Bemærk: Eksporten kører med RLS, så du skal være logget ind. CSV
        indeholder: id, created_at, severity, crop_name, latitude, longitude,
        photo_url, user_id.
      </p>
    </div>
  );
