...
type Palette = 'classic' | 'fire' | 'bluegreen';
...
const HEAT_GRADIENTS: Record<Palette, GradientMap> = {
  classic: {
    0.0: '#2c7bb6',
    0.2: '#abd9e9',
    0.4: '#ffffbf',
...
/* =========================================================
   Leaflet helpers (React Leaflet hooks)
========================================================= */
...
function MyLocationButton({
  ...
  return (
    <button
      type="button"
      onClick={handleClick}
      className="inline-flex items-center gap-2 text-xs px-2 py-1 rounded border hover:bg-slate-50"
      title="Zoom til min nuværende position"
    >
      📍 Min placering
    </button>
  );
...
export default function HazardReportsMapClient({
  hazardId,
}: {
  hazardId: string;
}) {
  ...
  const [heatPalette, setHeatPalette] = useState<Palette>('classic');
  ...
  useEffect(() => {
    try {
      ...
      const pick = <T extends string>(key: T, alt?: any) =>
        q[key] ?? saved[key] ?? alt;
      ...
      const qMin = pick('minSeverity');
      if (qMin) setMinSeverity(Number(qMin));
      const qCrop = pick('cropId');
      if (qCrop) setCropId(qCrop);
      const qMine = pick('onlyMine');
      if (qMine != null) setOnlyMine(qMine === 'true');

      const qShow = pick('heat');
      if (qShow != null)
        setShowHeatmap(String(qShow) === '1' || String(qShow) === 'true');
      const qRad = pick('hr');
      if (qRad) setHeatRadius(Number(qRad));
      const qBlur = pick('hb');
      if (qBlur) setHeatBlur(Number(qBlur));
      const qPal = pick('hp');
      if (qPal && ['classic', 'fire', 'bluegreen'].includes(qPal))
        setHeatPalette(qPal as Palette);
      ...
      const qFollow = pick('fm');
      if (qFollow != null)
        setFollowMe(String(qFollow) === '1' || String(qFollow) === 'true');
    } catch {
      /* ignore */
    }
    ...
    (async () => {
      try {
        ...
        let q = supabase
          .from('hazard_reports')
          .select(
            'id, latitude, longitude, created_at, severity, crop_id, photo_url, user_id, crops(name)',
          )
        ...
        const mapped = (data || []).map((r: any): ReportRow => {
          ...
          if (r.crops && !Array.isArray(r.crops))
            crops = { name: r.crops?.name };
          else if (Array.isArray(r.crops) && r.crops.length > 0)
            crops = { name: r.crops[0]?.name };
        ...
        for (const r of mapped)
          if (r.crop_id) uniq.set(r.crop_id, r.crops?.name || r.crop_id);
        setCropOptions(
          Array.from(uniq)
            .map(([id, name]) => ({ crop_id: id, name }))
        ...
        if (!center) {
          ...
          if (uid) {
            ...
            if (prof?.latitude != null && prof?.longitude != null)
              ctr = [prof.latitude, prof.longitude];
          }
          if (!ctr) {
            const first = mapped.find(
  ...
  useEffect(() => {
    ...
    const idx: Supercluster<any, any> = new Supercluster({
      radius: 60,
      maxZoom: 20,
    });
  ...
  function exportCsv(allFiltered: boolean) {
    ...
    a.href = url;
    a.download = `hazard_reports_${dateTag}${
      allFiltered ? '' : '_viewport'
    }.csv`;
  ...
  const gradient = HEAT_GRADIENTS[heatPalette] ?? HEAT_GRADIENTS.classic;
  const legendCss = gradientToCssLinear(gradient);
  ...
  useEffect(() => {
    ...
    const id = navigator.geolocation.watchPosition(
      ...
      (err) => {
        ...
        alert('Live-tracking fejlede: ' + err.message);
        setFollowMe(false);
      },
  ...
  async function flagReport(reportId: string) {
    ...
    if (error) {
      alert('Kunne ikke flagge: ' + error.message);
      return;
    }
  ...
  async function saveDroppedReport() {
    ...
    const uid = session.session?.user.id;
    if (!uid) {
      alert('Du skal være logget ind for at gemme en rapport.');
      return;
    }
    if (!tempCropId) {
      alert('Vælg en afgrøde.');
      return;
    }
    ...
    try {
      ...
      const cropName =
        cropOptions.find((c) => c.crop_id === tempCropId)?.name || 'Afgrøde';
    ...
    <div className="grid gap-3">
      ...
      <div className="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 p-3">
        <div className="flex flex-wrap items-end gap-3">
          ...
          <label
            ...
            title="Intensiteten vægtes efter alvorlighed: 1→0.2, 5→1.0. Farven viser tæthed × alvorlighed."
          >
            <input
              type="checkbox"
              checked={showHeatmap}
          ...
          <div className="flex items-center gap-3 ml-auto flex-wrap">
            ...
            />
            {locBusy && (
              <span className="text-xs opacity-70">Finder position…</span>
            )}

            <label
              className="flex items-center gap-2"
              title="Live-tracking: kortet følger din GPS-position."
            >
              <input
                type="checkbox"
                checked={followMe}
            ...
            <button
              ...
              className={`text-xs px-2 py-1 rounded border hover:bg-slate-50 ${
                dropMode ? 'bg-amber-50 border-amber-300' : ''
              }`}
            ...
            {showHeatmap && (
              <>
                <label className="grid gap-0.5">
                  ...
                  <select
                    value={heatPalette}
                    onChange={(e) => setHeatPalette(e.target.value as Palette)}
                    className="border px-2 py-1 rounded"
                  >
                ...
                <label className="grid gap-0.5">
                  ...
                  <input
                    ...
                    value={heatRadius}
                    onChange={(e) =>
                      setHeatRadius(Number(e.target.value) || 25)
                    }
                ...
                <label className="grid gap-0.5">
                  ...
                  <input
                    ...
                    max={50}
                    value={heatBlur}
                    onChange={(e) => setHeatBlur(Number(e.target.value) || 15)}
                    className="border px-2 py-1 rounded w-20"
                  />
                </label>
              </>
            ...
            <div className="flex items-center gap-2 pl-2 border-l">
              <button
                ...
                className="text-xs px-2 py-1 rounded border hover:bg-slate-50"
                title="Download CSV for alle rækker der matcher dine filtre"
              >
                Export (alle)
              </button>
              <button
                ...
                className="text-xs px-2 py-1 rounded border hover:bg-slate-50"
                title="Download CSV for punkter i det synlige kort-udsyn"
              >
                Export (udsnit)
              </button>
              <button
        ...
        {dropMode && (
          <div className="mt-2 text-[12px] text-amber-700 bg-amber-50 border border-amber-200 rounded px-2 py-1 inline-block">
            Drop nål er aktiv – klik på kortet for at vælge en position.{' '}
            <b>Træk nålen</b> for at finjustere.
          </div>
        )}
      </div>
      ...
      {!loading && !err && center && (
        <div className="w-full h-[420px] rounded-2xl overflow-hidden ring-1 ring-slate-200 relative">
          <MapContainer
            ...
            className="z-0"
          >
            {/* Sæt mapRef via context – kompatibelt med din react-leaflet version */}
            <MapRefSetter
              onMap={(map) => {
                mapRef.current = map;
              }}
            ...
            {/* Lille cirkel omkring center (kan fjernes) */}
            ...
            {showHeatmap ? (
              <HeatmapLayer
                ...
                blur={heatBlur}
                maxZoom={18}
                gradient={HEAT_GRADIENTS[heatPalette] ?? HEAT_GRADIENTS.classic}
              />
            ) : (
              <>
                {clusters.map((f: any) => {
                  ...
                  if (isCluster) {
                        ...
                        eventHandlers={{
                          click: (e) => {
                            ...
                            const expansionZoom = Math.min(
                              idx.getClusterExpansionZoom(f.id),
                              20,
                            );
                    ...
                    return (
                      <Marker
                        key={p.id}
                        position={[lat, lng]}
                        icon={pinIcon(p.severity)}
                      >
                        <Popup>
                          <div className="text-sm space-y-2">
                            <div>
                              <div className="font-medium">
                                {p.crop_name} · Alvor: {p.severity ?? '-'}
                              </div>
          ...
          {showHeatmap && (
            ...
            >
              <div className="text-[11px] mb-1 opacity-70">
                Intensitet (vægtet af alvorlighed)
              </div>
              <div
                style={{
                  width: 180,
              ...
              <p className="text-[11px] opacity-70 mt-1">
                Farveskalaen afspejler <em>tæthed × alvorlighed</em>.
                Alvorlighed vægter 1→0.2 … 5→1.0.
              </p>
            </div>
          )}
          ...
          {tempPin && (
            ...
            >
              ...
              <div className="text-xs opacity-70 mb-2">
                Lat: {tempPin.lat.toFixed(5)}, Lng: {tempPin.lng.toFixed(5)} (du
                kan trække nålen)
              </div>
              ...
              <div className="grid gap-2">
                <label className="grid gap-1 text-sm">
                  ...
                  <select
                    value={tempCropId}
                    onChange={(e) => setTempCropId(e.target.value)}
                    className="border px-2 py-1 rounded"
                  >
      ...
      {!loading && !err && (!reports || reports.length === 0) && (
        <div className="text-sm opacity-70">
          Ingen rapporter matcher dine filtre.
        </div>
      )}
    </div>
  );
