      ...
      try {
        ...
        for (let i = 0; i < hourlyTimes.length; i++) {
          ...
          if (
            ...
            frost = true;
            break; // vi har fundet første frosttime, stop
          }
        }
        setFrostRisk(frost);
      } catch {
  ...
  if (!coords) {
    return (
      <section className="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 p-4">
        ...
        <p className="text-sm">
          Ingen lokation sat. Gå til{' '}
          <a className="underline" href="/settings">
            Indstillinger
          </a>{' '}
          og udfyld latitude/longitude.
        </p>
      </section>
    );
  }
      ...
      {w.dailyTimes.length ? (
        <div className="mt-3 text-sm">
          <div className="font-medium mb-1">
            I dag ({fmtDay(w.dailyTimes[todayIdx] ?? '')})
          </div>
          <div className="inline-flex items-center gap-3">
            <div className="px-2 py-1 rounded bg-slate-100">
              Min{' '}
              {Number.isFinite(w.dailyTmin?.[todayIdx])
                ? Math.round(w.dailyTmin![todayIdx]!)
                : '—'}
              °C
            </div>
            <div className="px-2 py-1 rounded bg-slate-100">
              Max{' '}
              {Number.isFinite(w.dailyTmax?.[todayIdx])
                ? Math.round(w.dailyTmax![todayIdx]!)
                : '—'}
              °C
            </div>
          </div>
        ...
        <ul className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-3">
          {w.dailyTimes.slice(1, 5).map((d, i) => (
            <li key={d} className="p-2 rounded border border-slate-200 text-sm">
              ...
              <div className="mt-1">
                Min{' '}
                {Number.isFinite(w.dailyTmin?.[i + 1])
                  ? Math.round(w.dailyTmin![i + 1]!)
                  : '—'}
                ° • Max{' '}
                {Number.isFinite(w.dailyTmax?.[i + 1])
                  ? Math.round(w.dailyTmax![i + 1]!)
                  : '—'}
                °
              </div>
            </li>
        ...
        <div className="mt-3">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {w.hourlyTimes.slice(0, 12).map((t, i) => {
              const hh = new Date(t).toLocaleTimeString('da-DK', {
                hour: '2-digit',
              });
              const temp = Number.isFinite(w.hourlyTemps?.[i])
                ? Math.round(w.hourlyTemps![i]!)
                : null;
              return (
                <div
                  key={t}
                  className="min-w-[60px] p-2 rounded bg-slate-100 text-center text-sm"
                >
