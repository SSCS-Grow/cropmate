...
export default function WeatherHistoryChart() {
  ...
  const [thresholds, setThresholds] = useState<{ et0: number; rain: number }>({
    et0: 3.5,
    rain: 1.5,
  })
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      // Hent user_settings (ET0/Regn) + historik
      const [{ data: us }, { data, error }] = await Promise.all([
        supabase
          .from('user_settings')
    ...
    <section className="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 p-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs opacity-60">
          ET₀ (mm), Nedbør (mm), Tmax (°C) + vand-dage
        </span>
      </div>
        ...
        <div className="mt-3 w-full h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              {/* Vand-dages highlights – inkl. sidste dag */}
              {rows.map((r) => {
              ...
              {/* Tærskel-linjer (mm-aksen) – hentes fra user_settings */}
              <ReferenceLine
                yAxisId="mm"
                y={thresholds.et0}
                strokeDasharray="4 4"
                ...
                label={{
                  value: `ET₀ tærskel ${thresholds.et0} mm`,
                  position: 'right',
                  fontSize: 12,
                }}
              />
              <ReferenceLine
                ...
                ifOverflow="extendDomain"
                label={{
                  value: `Regn-skip ${thresholds.rain} mm`,
                  position: 'right',
                  fontSize: 12,
                }}
              />
      ...
      <p className="text-[11px] opacity-60 mt-2">
        Grå highlights markerer vand-dage (💧). Stiplede linjer viser ET₀-tærskel og regn-skip fra dine indstillinger.
      </p>
    </section>
  )
}
